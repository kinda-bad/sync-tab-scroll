...
test('narrow (360px): lobby, member settings modal, error toast fit', async ({ page, browser }) => {
  ...
  await member.goto('http://localhost:6001/');
  ...
  await member.getByRole('button', { name: 'Select' }).first().click(); // pick a part — auto-closes the modal
  await expect(member.getByRole('dialog', { name: 'Song & part' })).not.toBeVisible();
  ...
  await toastPage.goto('http://localhost:6001/');
