...
- [x] T001 [artifacts: infrastructure] Create `.github/workflows/ci.yml`
...
- [x] T002 [artifacts: infrastructure] Add a `pnpm check` step (root
  script, fans out to every workspace package's own `check` script) as
  its own named step, after T001's install step.
- [x] T003 [artifacts: infrastructure] Add test-suite steps, each its own
...
- [x] T004 [artifacts: infrastructure] Add a Playwright browser install
...
- [x] T005 Do NOT add a `pnpm check:env` (or equivalent) step anywhere in
