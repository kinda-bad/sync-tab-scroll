...
- [ ] T010 [artifacts: pipeline, datamodel] Implement the Publish step: write a `catalog/<song-slug>/` directory containing the validated `.gp` file (served as-is, no transformed copy), the generated `.lrc`, and a `meta.json` holding `lyricsTrackIndex`, `lyricsLineIndex`, and `lyricLineBreaks` (or nulls, per whichever path ran).
