...
| playbackSource | 'synth' \| 'recording' | Which audio every participant in this session plays: the alphaTab synth (default) or the song's operator-supplied recording. Host-only toggle, same pattern as `countInEnabled`/`spotlightMode`. **Session-scoped, deliberately not per-participant** (`sync-tabs-to-real-audio`): a session mixing synth and recording listeners cannot be kept in audible sync, because aligning two clients' *reported* positions yields audible separation equal to the difference in their reported-vs-real-audio latencies — ~0 for a recording client (measured) but unknown and plausibly ~275ms for a synth one, which alphaTab 1.8.3 does not expose. Making source a session property removes that failure mode by construction (`research-recording-mode-drift-2026-07-19-b7c2.md`). Resets to `'synth'` when the selected song changes or the new song isn't recording-capable |
